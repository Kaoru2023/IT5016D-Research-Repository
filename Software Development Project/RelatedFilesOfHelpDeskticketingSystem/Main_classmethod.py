    ...
    def respond_ticket(self):
        try:
            ...
            if (ticket.ticket_number == ticket_number
                and (ticket.status == Ticket_classmethod.Ticket.Status.Open
                     or ticket.status == Ticket_classmethod.Ticket.Status.Reopened)):
...
##ここからコメントチェック
    ...
    def reopen_ticket(self):
        try:
            ...
            ticket = self.all_tickets[ticket_number]
            if (ticket.ticket_number == ticket_number
                and ticket.status == Ticket_classmethod.Ticket.Status.Closed):
    ...
    def show_ticket_stats(self):
        stats = Ticket_classmethod.Ticket.get_ticket_stats()
        print("-----------------------------------")
        print(f"Submitted Tickets: {stats[0] + stats[1]}",
              f"Resolved Tickets: {stats[1]}",
              f"Open Tickets: {stats[0]}",
              sep="\n"
              )
